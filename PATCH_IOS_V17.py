...
def patch_prsv_ios_picker():
    ...
    if not re.search(r"saveFile\.accept\s*=\s*[\"']\.prsv[\"'];", verify):
        raise SystemExit('ERROR: .prsv accept filter not found after patch')
    for needle in ('document.body.appendChild(saveFile);', 'saveFile.style.opacity = "0";', 'saveFile.click();', 'reader.readAsText(selectedFile);'):
        if needle not in verify:
            raise SystemExit(f'ERROR: verification failed: {needle}')
    print('=== iOS PRSV picker fix ===')
    print('style replacement count:', n_style)
...
def main():
    ...
    print('DONE: ZA-DIGI Rouge iOS v17 patch applied.')
