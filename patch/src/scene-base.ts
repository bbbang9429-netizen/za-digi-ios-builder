...
export class SceneBase extends Phaser.Scene {
  ...
  public loadImage(key: string, folder: string, filename = `${key}.png`): this {
    // Capacitor/WebView does not reliably normalize paths containing a double slash.
    // The title logo, fake logo and snow assets are loaded with an empty folder,
    // so build a clean images/<file> path instead of images//<file>.
    const imagePath = folder ? `images/${folder}/${filename}` : `images/${filename}`;
    this.load.image(key, getCachedUrl(imagePath));
