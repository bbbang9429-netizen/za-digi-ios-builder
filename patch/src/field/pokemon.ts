...
export abstract class Pokemon extends Phaser.GameObjects.Container {
  ...
  public async changeForm(formChange: SpeciesFormChange): Promise<void> {
    // Offline custom starter editor: custom ability/passive are permanent per-Pokemon
    // overrides and must survive temporary/permanent form changes (Mega, G-Max, etc.).
    const persistentCustomAbility = this.customPokemonData.ability;
    const persistentCustomPassive = this.customPokemonData.passive;
    ...
    // A form may have a different native ability table, but an editor override
    // intentionally takes precedence over that table. Re-assert it explicitly.
    this.customPokemonData.ability = persistentCustomAbility;
    this.customPokemonData.passive = persistentCustomPassive;
    if (persistentCustomPassive != null && persistentCustomPassive !== -1) {
      this.passive = true;
    }
...
export class PlayerPokemon extends Pokemon {
  ...
  evolve(evolution: SpeciesFormEvolution | null, preEvolution: PokemonSpeciesForm): Promise<void> {
    ...
    // Offline custom starter editor: evolution normally changes the species/form
    // ability table. The user-selected custom ability/passive belongs to this
    // individual Pokemon, so preserve it across every normal evolution.
    const persistentCustomAbility = this.customPokemonData.ability;
    const persistentCustomPassive = this.customPokemonData.passive;
    ...
    return new Promise(resolve => {
      ...
      // Keep the editor overrides even when the evolved species has completely
      // different native abilities/passives. This also persists into session saves
      // because customPokemonData is serialized with the Pokemon.
      this.customPokemonData.ability = persistentCustomAbility;
      this.customPokemonData.passive = persistentCustomPassive;
      if (persistentCustomPassive != null && persistentCustomPassive !== -1) {
        this.passive = true;
  ...
  changeForm(formChange: SpeciesFormChange): Promise<void> {
    // Preserve per-Pokemon editor overrides across player form changes, including
    // Mega Evolution, Gigantamax/Eternamax and reversible item-driven forms.
    const persistentCustomAbility = this.customPokemonData.ability;
    const persistentCustomPassive = this.customPokemonData.passive;
