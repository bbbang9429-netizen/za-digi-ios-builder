...
export abstract class ApiBase {
  ...
  protected async doFetch(path: string, config: DoFetchConfig): Promise<Response> {
    // The standalone app build is intentionally serverless. Prevent any accidental
    // API traffic even if an online-only UI path is reached.
    if (import.meta.env.VITE_BYPASS_LOGIN === "1") {
      return Promise.reject(new Error(`Network API disabled in standalone app build: ${path}`));
    }
