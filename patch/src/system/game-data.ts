...
// ZA-DIGI custom starter species that are absent from regular/upstream PokeRogue saves.
// When such a save is imported, v6 unlocks all of these as selectable starters.
const ZA_DIGI_IMPORTED_STARTER_UNLOCKS: readonly SpeciesId[] = [
  SpeciesId.BOTAMON,
  SpeciesId.BETAMON,
  SpeciesId.PALMON,
  SpeciesId.TSUNOMON,
...
export class GameData {
  ...
  private initParsedSystem(systemData: SystemSaveData): void {
    ...
    if (systemData.starterData) {
      // Forward-compatibility for imported saves created by a newer game build.
      // Keep this build's complete starter roster, then merge progress only for
      // starter IDs that exist in the current build. This prevents newer saves
      // from replacing starterData wholesale and leaving current custom starters
      // undefined (which crashes the starter-select screen).
      const importedStarterData = systemData.starterData;
      this.initStarterData();
      for (const speciesId of Object.keys(importedStarterData)) {
        if (!Object.prototype.hasOwnProperty.call(this.starterData, speciesId)) {
          continue;
        }
        this.starterData[speciesId] = {
          ...this.starterData[speciesId],
          ...importedStarterData[speciesId],
        };
      }
    } else {
      this.initStarterData();
    ...
    // Merge only species known by this build. Newer saves can contain species
    // IDs that do not exist here; retaining them is unnecessary and can make
    // older/custom UI paths attempt to resolve unknown species.
    if (systemData.dexData) {
      for (const speciesId of Object.keys(systemData.dexData)) {
        if (!Object.prototype.hasOwnProperty.call(this.dexData, speciesId)) {
          continue;
        }
        this.dexData[speciesId] = Object.assign(this.dexData[speciesId], systemData.dexData[speciesId]);
      }
    }

    // ZA-DIGI offline import compatibility: ordinary PokeRogue saves do not
    // contain the custom Digimon starter entries.  The user explicitly wants
    // all ZA-DIGI-only starters unlocked after importing such a save.
    //
    // We only synthesize data for entries missing from the imported starterData;
    // existing progress for any ZA-DIGI starter is never overwritten.
    if (systemData.starterData) {
      const defaultUnlockedAttr =
        DexAttr.NON_SHINY | DexAttr.MALE | DexAttr.FEMALE | DexAttr.DEFAULT_VARIANT | DexAttr.DEFAULT_FORM;

      for (const speciesId of ZA_DIGI_IMPORTED_STARTER_UNLOCKS) {
        if (Object.prototype.hasOwnProperty.call(systemData.starterData, speciesId)) {
          continue;
        }
        ...
        dexEntry.seenAttr |= defaultUnlockedAttr;
        dexEntry.caughtAttr |= defaultUnlockedAttr;
        dexEntry.seenCount = Math.max(dexEntry.seenCount, 1);
        dexEntry.caughtCount = Math.max(dexEntry.caughtCount, 1);

        // Give a deterministic valid nature and sane starter IVs when the
        // imported save had no data for this custom species.
        if (!dexEntry.natureAttr) {
          dexEntry.natureAttr = 1 << (Nature.HARDY + 1);
        }
        if (dexEntry.ivs.every(iv => iv === 0)) {
          dexEntry.ivs.fill(15);
        }

        // A caught starter with abilityAttr=0 can render as unlocked but fail
        // later when the starter UI resolves its selectable ability.
        if (!starterEntry.abilityAttr) {
  ...
  public importData(dataType: GameDataType, slotId = 0): void {
    ...
    saveFile.type = "file";
    saveFile.accept = ".prsv,text/plain,application/octet-stream";
    saveFile.style.display = "none";
    document.body.appendChild(saveFile);

    const releaseInputs = () => globalScene.inputController?.loseFocus();
    ...
    // Defer opening the Android document picker until the touch that selected
