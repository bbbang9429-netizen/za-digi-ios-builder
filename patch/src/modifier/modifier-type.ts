...
function getNewModifierTypeOption(
  ...
  if (tier === undefined) {
    /**
     * ZA-DIGI offline v10 reward-tier rebalance.
     *
     * Base probabilities BEFORE shiny/luck tier upgrades:
     *   COMMON  65%
     *   GREAT   19%
     *   ULTRA   10%
     *   ROGUE    5%
     *   MASTER   1%
     *
     * A 10,000-point roll is used so the requested percentages are exact.
     * The existing shiny/luck upgrade logic below is intentionally preserved,
     * so party luck can still promote the initially rolled tier afterwards.
     */
    const tierValue = randSeedInt(10000);
    if (!upgradeCount) {
      upgradeCount = 0;
    }
    ...
    if (tierValue >= 3500) {
      tier = ModifierTier.COMMON; // 6500 / 10000 = 65%
    } else if (tierValue >= 1600) {
      tier = ModifierTier.GREAT; // 1900 / 10000 = 19%
    } else if (tierValue >= 600) {
      tier = ModifierTier.ULTRA; // 1000 / 10000 = 10%
    } else if (tierValue >= 100) {
      tier = ModifierTier.ROGUE; // 500 / 10000 = 5%
    } else {
      tier = ModifierTier.MASTER; // 100 / 10000 = 1%
    }
