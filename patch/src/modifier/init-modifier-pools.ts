...
function initGreatModifierPool() {
  modifierPool[ModifierTier.GREAT] = [
    ...
    new WeightedModifierType(
      ...
      weightFromWave(55, OFFLINE_FORM_CHANGE_BASE_WEIGHT),
      OFFLINE_FORM_CHANGE_BASE_WEIGHT,
    ),
    new WeightedModifierType(
      modifierTypes.MAP,
...
/**
 * Offline build balance tweak:
 * Make evolution/form-change progression items substantially easier to find
 * without changing the shiny/luck tier-up system itself.
 *
 * These six entries all live in the ULTRA reward pool. Multiplying only their
 * intra-tier weights keeps tier generation unchanged while making the desired
 * item families roughly 10x as likely relative to other ULTRA rewards whenever
 * they are eligible to appear.
 */
const OFFLINE_FORM_CHANGE_WEIGHT_MULTIPLIER = 10;
const OFFLINE_FORM_CHANGE_BASE_WEIGHT = 2000 * OFFLINE_FORM_CHANGE_WEIGHT_MULTIPLIER;
