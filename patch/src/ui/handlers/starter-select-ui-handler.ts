  ...
  */
  ...
  processInput(button: Button): boolean {
    ...
    if (button === Button.SUBMIT) {
      ...
      // ZA-DIGI offline custom starter editor. Uses the existing text-entry modal as a search box,
      // then shows matching implemented abilities/moves. These choices are persisted in starter preferences
      // and copied onto the individual PlayerPokemon when a run starts.
      const decodeSearchText = (encoded: string): string => {
        try {
      ...
      const localizedEditorLabel = (ko: string, en: string): string =>
        i18next.language?.toLowerCase().startsWith("ko") ? ko : en;

      const normalizeEditorSearch = (text: string): string => text.normalize("NFKC").trim().toLocaleLowerCase();

      const getEditorAutocompleteValues = (
        kind: "ability" | "passive" | "eggMove",
        rawQuery: string,
      ): string[] => {
        const query = normalizeEditorSearch(rawQuery);
        if (!query) {
          return [];
        }

        const names = kind === "eggMove"
          ? allMoves
              .filter(m => !!m && !!m.name && m.id !== 0 && !m.name.endsWith(" (N)"))
              .map(m => m.name)
          : allAbilities
              .filter(a => !!a && a.id !== AbilityId.NONE && !a.unimplemented && !!a.name)
              .map(a => a.name);

        // Prefix matches are the useful autocomplete case (e.g. "천하" -> "천하장사").
        // Substring matches remain available underneath for forgiving search.
        const uniqueMatches = [...new Set(names.filter(name => normalizeEditorSearch(name).includes(query)))];
        uniqueMatches.sort((a, b) => {
          const aPrefix = normalizeEditorSearch(a).startsWith(query) ? 0 : 1;
          const bPrefix = normalizeEditorSearch(b).startsWith(query) ? 0 : 1;
          if (aPrefix !== bPrefix) {
      ...
      // Return cleanly from the offline editor. The search flow temporarily uses the
      // rename modal, autocomplete overlay, option-select menu, and the starter message box.
      // Because some of those layers are opened without clearing the previous mode, simply
      // switching back to STARTER_SELECT can leave stale windows visible. Explicitly clear
      // every editor-owned layer and the message prompt before restoring starter selection.
      const finishOfflineEditorFlow = (): void => {
        this.moveInfoOverlay.clear();
        globalScene.ui.hideTooltip();
        this.clearText();

        // Defensive cleanup for stale modal/overlay layers (not necessarily the current mode).
        ui.handlers[UiMode.RENAME_POKEMON]?.clear();
        ui.handlers[UiMode.AUTO_COMPLETE]?.clear();
        ui.resetModeChain();

        void ui.setMode(UiMode.STARTER_SELECT).then(() => {
          this.clearText();
        });
      };
      ...
      const openSearchPrompt = (
        ...
        const promptText =
          kind === "ability"
            ? localizedEditorLabel("바꿀 특성의 이름을 검색하세요.", "Search for an active ability.")
            : kind === "passive"
              ? localizedEditorLabel("바꿀 패시브의 이름을 검색하세요.", "Search for a passive ability.")
              : localizedEditorLabel(`알기술 ${Number(eggMoveSlot) + 1}번 슬롯에 넣을 기술을 검색하세요.`, `Search for a move for custom egg-move slot ${Number(eggMoveSlot) + 1}.`);

        ui.setMode(UiMode.STARTER_SELECT).then(() => {
          ui.showText(promptText, null, () => {
            ui.setModeWithoutClear(
              UiMode.RENAME_POKEMON,
              {
                buttonActions: [
                  (encodedText: string) => {
                    const query = decodeSearchText(encodedText).toLocaleLowerCase();
                    if (!query) {
                      finishOfflineEditorFlow();
                      return;
                    }

                    if (kind === "eggMove") {
                      const matches = allMoves
                        .filter(m => !!m && !!m.name && m.id !== 0 && !m.name.endsWith(" (N)"))
                        .filter(m => m.name.toLocaleLowerCase().includes(query))
                        .slice(0, 60);
                      ...
                      if (!matches.length) {
                        ...
                        this.clearText();
                        ui.setMode(UiMode.STARTER_SELECT).then(() => {
                          ui.showText(localizedEditorLabel("검색 결과가 없습니다.", "No matching move found."));
                        });
                        return;
                      }
                      ...
                      ui.setMode(UiMode.STARTER_SELECT).then(() => {
                        this.clearText();
                        ui.showText(localizedEditorLabel("알기술을 선택하세요.", "Choose a custom egg move."), null, () => {
                          this.moveInfoOverlay.show(matches[0]);
                          ui.setModeWithoutClear(UiMode.OPTION_SELECT, {
                            options: matches
                              .map(move => ({
                                label: move.name,
                                handler: () => {
                                  const slot = eggMoveSlot ?? 0;
                                  const baseEggMoves = speciesEggMoves.hasOwnProperty(this.lastSpecies.speciesId)
                                    ? [...speciesEggMoves[this.lastSpecies.speciesId as keyof typeof speciesEggMoves]]
                                    : [];
                                  const customEggMoves = [...(starterAttributes.customEggMoves ?? baseEggMoves)].slice(0, 4);
                                  while (customEggMoves.length < 4) {
                                    customEggMoves.push(0 as MoveId);
                                  }
                                  customEggMoves[slot] = move.id as MoveId;
                                  starterAttributes.customEggMoves = customEggMoves;
                                  originalStarterAttributes.customEggMoves = [...customEggMoves];

                                  // Treat custom egg-move slots as unlocked in the offline editor.
                                  persistentStarterData.eggMoves |= 1 << slot;
                                  starterData.eggMoves = persistentStarterData.eggMoves;

                                  // If this species has already been added to the starter party, keep the
                                  // per-run marker in sync. setSpeciesDetails() below recomputes the current
                                  // starter moveset, then updateSelectedStarterMoveset() copies it onto the
                                  // already-selected Starter object.
                                  const [alreadyInParty, partyIndex] = this.isInParty(this.lastSpecies);
                                  if (alreadyInParty) {
                                    this.starters[partyIndex].customEggMoves = [...customEggMoves];
                                  }
                                  ...
                                  if (alreadyInParty) {
                                    this.updateSelectedStarterMoveset(this.lastSpecies.speciesId);
                                  }
                                  finishOfflineEditorFlow();
                                  return true;
                                },
                                onHover: () => this.moveInfoOverlay.show(move),
                              }))
                              .concat({
                    ...
                    if (!matches.length) {
                      ...
                      this.clearText();
                      ui.setMode(UiMode.STARTER_SELECT).then(() => {
                        ui.showText(localizedEditorLabel("검색 결과가 없습니다.", "No matching ability found."));
                      });
                      return;
                    }
                    ...
                    ui.setMode(UiMode.STARTER_SELECT).then(() => {
                      ...
                      ui.showText(
                        kind === "ability"
                          ? localizedEditorLabel("바꿀 특성을 선택하세요.", "Choose the active ability.")
                          : localizedEditorLabel("바꿀 패시브를 선택하세요.", "Choose the passive ability."),
                        null,
                        () => {
                          globalScene.ui.showTooltip(matches[0].name, matches[0].description, true);
                          ui.setModeWithoutClear(UiMode.OPTION_SELECT, {
                            options: matches
                              .map(ability => ({
                                label: ability.name,
                                handler: () => {
                                  const [alreadyInParty, partyIndex] = this.isInParty(this.lastSpecies);
                                  if (kind === "ability") {
                                    starterAttributes.customAbility = ability.id;
                                    originalStarterAttributes.customAbility = ability.id;
                                    this.abilityCursor = 0;
                                    if (alreadyInParty) {
                                      this.starters[partyIndex].customAbility = ability.id;
                                    }
                                  } else {
                                    starterAttributes.customPassive = ability.id;
                                    originalStarterAttributes.customPassive = ability.id;
                                    ...
                                    if (alreadyInParty) {
                                      this.starters[partyIndex].customPassive = ability.id;
                                      this.starters[partyIndex].passive = true;
                                    }
                                    globalScene.gameData.saveSystem();
                                  }
                                  globalScene.ui.hideTooltip();
                              ...
                              .concat({
                                ...
                                onHover: () => globalScene.ui.hideTooltip(),
                              }),
                            supportHover: true,
                            maxOptions: 8,
                            yOffset: 19,
                ...
                autocompleteValues: (query: string) => getEditorAutocompleteValues(kind, query),
                autocompleteMaxOptions: 5,
              },
              "",
            );
          });
      ...
      if (button === Button.ACTION) {
        if (!this.speciesStarterDexEntry?.caughtAttr) {
          ...
          // Offline editor options: arbitrary active ability, passive, and four custom egg-move slots.
          options.push({
            label: localizedEditorLabel("[검색] 특성 변경", "[Search] Change Ability"),
            handler: () => {
              openSearchPrompt("ability");
              return true;
            },
          });
          options.push({
            label: localizedEditorLabel("[검색] 패시브 변경", "[Search] Change Passive"),
            handler: () => {
              openSearchPrompt("passive");
              return true;
            },
          });
          options.push({
            label: localizedEditorLabel("[검색] 알기술 변경", "[Search] Change Egg Moves"),
            handler: () => {
              const effectiveEggMoves = starterAttributes.customEggMoves
                ?? (speciesEggMoves.hasOwnProperty(this.lastSpecies.speciesId)
                  ? [...speciesEggMoves[this.lastSpecies.speciesId as keyof typeof speciesEggMoves]]
                  : []);
              ui.setMode(UiMode.STARTER_SELECT).then(() => {
                ui.showText(localizedEditorLabel("바꿀 알기술 슬롯을 선택하세요.", "Choose an egg-move slot."), null, () => {
                  ui.setModeWithoutClear(UiMode.OPTION_SELECT, {
                    options: [0, 1, 2, 3]
                      .map(slot => ({
                        label: `${slot + 1}. ${effectiveEggMoves[slot] ? allMoves[effectiveEggMoves[slot]]?.name ?? "-" : "-"}`,
                        handler: () => {
                          openSearchPrompt("eggMove", slot);
                          return true;
                        },
                      }))
                      .concat({
          ...
          if (starterAttributes.customAbility != null || starterAttributes.customPassive != null || starterAttributes.customEggMoves?.length) {
            options.push({
              label: localizedEditorLabel("커스텀 설정 초기화", "Reset Custom Overrides"),
              handler: () => {
                delete starterAttributes.customAbility;
                delete starterAttributes.customPassive;
                delete starterAttributes.customEggMoves;
                delete originalStarterAttributes.customAbility;
                delete originalStarterAttributes.customPassive;
                delete originalStarterAttributes.customEggMoves;
                saveStarterPreferences(this.originalStarterPreferences);
                this.setSpeciesDetails(this.lastSpecies);
                ui.setMode(UiMode.STARTER_SELECT);
                return true;
          ...
          ui.setModeWithoutClear(UiMode.OPTION_SELECT, {
            options,
            // The offline editor adds several entries to this action menu.
            // Keep the window inside the game viewport and let the stock
            // OPTION_SELECT handler scroll the list with the cursor.
            maxOptions: 8,
            yOffset: 47,
          });
          success = true;
  ...
  addToParty(
    ...
    const starter = {
      ...
      teraType,
      ivs: dexEntry.ivs,
      customAbility: this.starterPreferences[species.speciesId]?.customAbility,
      customPassive: this.starterPreferences[species.speciesId]?.customPassive,
      customEggMoves: this.starterPreferences[species.speciesId]?.customEggMoves?.slice(),
    };
  ...
  setSpeciesDetails(species: PokemonSpecies, options: SpeciesDetails = {}, save = true): void {
    ...
    if (species) {
      ...
      if (dexEntry.caughtAttr) {
        const customStarterPrefs = this.starterPreferences[species.speciesId];
        let ability: Ability;
        if (customStarterPrefs?.customAbility != null) {
          ability = allAbilities[customStarterPrefs.customAbility];
        ...
        const isHidden = customStarterPrefs?.customAbility == null && abilityIndex === (this.lastSpecies.ability2 ? 2 : 1);
        this.pokemonAbilityText
        ...
        const passiveAbility = customStarterPrefs?.customPassive != null
          ? allAbilities[customStarterPrefs.customPassive]
        ...
        const customEggMoves = this.starterPreferences[species.speciesId]?.customEggMoves;
        const effectiveEggMoves = customEggMoves?.length
          ? customEggMoves
          : speciesEggMoves.hasOwnProperty(species.speciesId)
            ? speciesEggMoves[species.speciesId as keyof typeof speciesEggMoves]
            : [];
        for (let em = 0; em < Math.min(4, effectiveEggMoves.length); em++) {
          if (effectiveEggMoves[em] && (customEggMoves?.length || starterDataEntry.eggMoves & (1 << em))) {
            this.speciesStarterMoves.push(effectiveEggMoves[em]);
          }
        }
        ...
        const availableStarterMoves = this.speciesStarterMoves.concat(
          effectiveEggMoves.filter((m: MoveId, em: number) => !!m && (customEggMoves?.length || starterDataEntry.eggMoves & (1 << em))),
        );
    ...
    const customEggMovePrefs = species ? this.starterPreferences[species.speciesId]?.customEggMoves : undefined;
    const displayedEggMoves = species
      ? customEggMovePrefs?.length
        ? customEggMovePrefs
        : speciesEggMoves.hasOwnProperty(species.speciesId)
          ? speciesEggMoves[species.speciesId as keyof typeof speciesEggMoves]
          : []
      : [];
    const hasEggMoves = !!species && displayedEggMoves.length > 0;
    let eggMoves = 0;
    if (species) {
    ...
    for (let em = 0; em < 4; em++) {
      const eggMove = hasEggMoves && displayedEggMoves[em] ? allMoves[displayedEggMoves[em]] : null;
      const eggMoveUnlocked = eggMove && (!!customEggMovePrefs?.length || eggMoves & (1 << em));
