...
def patch_prsv_ios_picker():
    ...
    # Replace display:none if present; detached/hidden file inputs are unreliable in WKWebView.
    s, n_style = re.subn(
    ...
    # Current ZA-DIGI source creates and configures saveFile but does not call click().
    # Insert DOM attach + synchronous click only after the onchange/FileReader handler is set.
    if 'document.body.appendChild(saveFile);' not in s or 'saveFile.click();' not in s:
        anchor = '// "Import Data" has been fully released. This prevents a stuck virtual key.'
        idx = s.find(anchor)
        if idx < 0:
            raise SystemExit('ERROR: import completion anchor not found')
        line_start = s.rfind('\n', 0, idx) + 1
        indent_match = re.match(r'[ \t]*', s[line_start:idx])
        indent = indent_match.group(0) if indent_match else '    '
        insert = ''
        if 'document.body.appendChild(saveFile);' not in s:
            insert += indent + 'document.body.appendChild(saveFile);\n'
        if 'saveFile.click();' not in s:
            insert += indent + 'saveFile.click();\n'
    ...
    print('saveFile.click verified: YES')
...
def main():
    ...
    print('DONE: ZA-DIGI Rouge iOS v17.2 patch applied.')
